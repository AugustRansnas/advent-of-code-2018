...
(defn day1-b
  {:test (fn []
           (is= (day1-b [+1 -1]) 0)
           (is= (day1-b [+3 +3 +4 -2 -4]) 10)
           (is= (day1-b [-6 +3 +8 +5 -6]) 5)
           (is= (day1-b [+7 +7 -2 -7 -4]) 14))}
  [frequencies]
  (reduce (fn [existing-values new-value]
              (if (contains? existing-values new-value)
                (reduced new-value)
                (conj existing-values new-value)))
          #{0}
